...
void QtEventExecutor::install() {
    ...
    DEBUG(D_EXECUTOR, "Event Executor installed.");
}
...
void QtEventExecutor::handleNewTestItemReceived(DataModel::TestItem *ti) {
    ...
    if (f_executing_ == false) {
        // no event executing
        DEBUG(D_EXECUTOR, "Stop state. No event executing.");
        return;
    }
    // if process is paused...
    else if (f_executing_ && f_paused_) {
        // no event executing
        DEBUG(D_EXECUTOR, "Pause state. No event executing.");
        return;
    }
...
void QtEventExecutor::executeMousePressEvent(QOE::QOE_MousePress *qoe) {
    ...
    if (widget == NULL) {
        DEBUG(D_ERROR, "Problem getting Widget: " << qoe->widget());
    }
    else {
        QTDEBUGT(D_EXECUTOR, "Retrieved widget:" << widget);
    }

    /// Note that the widget might be NULL. Please, be careful!!

    QTDEBUG("Pre execution:" << widget);
    _preExecutionWithMouseMove(qoe, widget);
    QTDEBUGT(D_EXECUTOR, "Execute:" << widget);
    qoe->execute(widget);
    QTDEBUGT(D_EXECUTOR, "Post execution:" << widget);
    _postExecution(qoe, widget);

    QTDEBUGT(D_EXECUTOR, "Exit:" << widget);
}
...
void QtEventExecutor::executeKeyPressEvent(QOE::QOE_KeyPress *qoe) {
    ...
    QWidget *widget = _getWidget(qoe);
    if (widget == NULL) {
        DEBUG(D_ERROR, "Problem getting Widget: "
              << qoe->widget());
    }
    else {
        QTDEBUGT(D_EXECUTOR, "Retrieved widget:" << widget);
...
/*
void QtEventExecutor::_simulateMouseMove ( const QPoint& pBegin, const QPoint&
pEnd )
{
    int ix = pBegin.x();
    int iy = pBegin.y();
...
void QtEventExecutor::_simulateMouseMove(const QPoint &pBegin,
    ...
    */
    ...
    for (uint i = 1; i <= MOUSE_MOVE_STEPS; i++) {
        ...
        if (hoverOnWidget) {
            QMouseEvent me(QEvent::MouseMove, hoverOnWidget->mapFromGlobal(current),
                           Qt::NoButton, Qt::LeftButton,
                           QApplication::keyboardModifiers());
    ...
    QCursor::setPos(pEnd);
    if (hoverOnWidget) {
        QMouseEvent me(QEvent::MouseMove, hoverOnWidget->mapFromGlobal(pEnd),
                       Qt::NoButton, Qt::LeftButton,
                       QApplication::keyboardModifiers());
...
void QtEventExecutor::_preExecutionNoMouse(QOE::QOE_Base *qoe,
                                           QWidget *widget) {
    assert(qoe);
...
void QtEventExecutor::_preExecutionWithMouseMove(QOE::QOE_Base *qoe,
                                                 QWidget *widget) {
    assert(qoe);
...
void QtEventExecutor::_preExecutionWithMouseHover(QOE::QOE_Base *qoe,
                                                  QWidget *widget) {
    assert(qoe);
    ...
    if (widget != NULL) {
        ...
        // widget); _last_mouse_pos = widget->mapToGlobal ( qoe->position() );
        QPoint target = qoe->adaptedGlobalPosition(widget);
        _simulateMouseMove(_last_mouse_pos, target, widget);
        _last_mouse_pos = target;
    }
