...
void ProcessControl::initialize() {
  ...
  dataModel_manager_->addDataModelAdapter(dataModel_adapter_->id(),
                                          dataModel_adapter_);
  assert(
      dataModel_manager_->setCurrentDataModelAdapter(dataModel_adapter_->id()));
  ...
  connect(_comm.get(), SIGNAL(error(const std::string &)), this,
          SLOT(slot_handleCommError(const std::string &)));
  connect(_comm.get(), SIGNAL(receivedTestItem(DataModel::TestItem *)), this,
          SLOT(handleControlSignaling(DataModel::TestItem *)));
  ...
  connect(preloading_action_, SIGNAL(applicationClosed(int)), this,
          SLOT(slot_handleApplicationClosed(int)));
...
DataModel::TestSuite *
ProcessControl::loadTestSuiteObject(const std::string &file) {
  try {
    return dataModel_manager_->getCurrentDataModelAdapter()->file2testSuite(
        file);
  }
...
/// process state
///
ProcessControl::OHTProcessState ProcessControl::state() const { return state_; }
