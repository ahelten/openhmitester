...
HMITesterControl::HMITesterControl(PreloadingAction *pa, DataModelAdapter *dma,
                                   QWidget *parent)
    : QMainWindow(parent) {
    // create the GUI
    ui.setupUi(this);
...
void HMITesterControl::_initializeForm() {
    ...
    connect(this, SIGNAL(form_initState()), this, SLOT(_form_initState()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(form_stopState()), this, SLOT(_form_stopState()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(form_playState()), this, SLOT(_form_playState()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(form_recState()), this, SLOT(_form_recState()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(form_pausePlayState()), this,
            SLOT(_form_pausePlayState()), Qt::QueuedConnection);
    connect(this, SIGNAL(form_pauseRecState()), this, SLOT(_form_pauseRecState()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(form_playbackStatus(int)), this,
            SLOT(_form_playbackStatus(int)), Qt::QueuedConnection);
    connect(this, SIGNAL(form_recordingStatus(int)), this,
            SLOT(_form_recordingStatus(int)), Qt::QueuedConnection);
...
void HMITesterControl::_initializeMenu() {
    ...
    connect(ui.action1x, SIGNAL(triggered(bool)), this,
            SLOT(action_speed1x_triggered()));
    connect(ui.action05x, SIGNAL(triggered(bool)), this,
            SLOT(action_speed05x_triggered()));
    connect(ui.action2x, SIGNAL(triggered(bool)), this,
            SLOT(action_speed2x_triggered()));
    connect(ui.action4x, SIGNAL(triggered(bool)), this,
            SLOT(action_speed4x_triggered()));
    connect(ui.actionKeepAlive, SIGNAL(triggered(bool)), this,
            SLOT(action_keepAlive_triggered(bool)));
    connect(ui.actionShowTesterOnTop, SIGNAL(triggered(bool)), this,
            SLOT(action_showTesterOnTop_triggered(bool)));
    connect(ui.action_Open, SIGNAL(triggered(bool)), this,
            SLOT(action_open_triggered()));
    connect(ui.action_Edit, SIGNAL(triggered(bool)), this,
            SLOT(action_edit_triggered()));
    connect(ui.action_New, SIGNAL(triggered(bool)), this,
            SLOT(action_new_triggered()));
    connect(ui.action_Close, SIGNAL(triggered(bool)), this,
            SLOT(action_close_triggered()));
    connect(ui.action_Exit, SIGNAL(triggered(bool)), this,
            SLOT(action_exit_triggered()));
...
void HMITesterControl::tb_rec_clicked() {
    ...
    if (_processControl->state() == ProcessControl::STOP) {
        ...
        if (result) {
            ...
            bool ok = _processControl->recordTestCase(
                              newtcd.getTestcaseName().toStdString());
            ...
            if (!ok) {
                ...
                bool cont = QtUtils::showOkCancelDialog(
                                    QString("TestCase '" + newtcd.getTestcaseName() +
                                            "' already exists. Do you want to redefine it?"));
                // if the testCase has to be redefined
                if (cont) {
                    ok = _processControl->recordExistingTestCase(
                                 newtcd.getTestcaseName().toStdString());
                    // if an error occurs...
                    if (!ok) {
                        QtUtils::newErrorDialog(
                                "The new TestCase cannot be created. Wrong data.");
                        return;
                    }
                }
                // if not...
...
void HMITesterControl::action_open_triggered() {
    ...
    QString path = "";
    path = QtUtils::openFileDialog(
                   "Please, select the file that contains the TestSuite:", lastOpenDir,
                   "*." OHT_FILE_EXTENSION);
    if (path == "")
        return;
...
void HMITesterControl::action_edit_triggered() {
    ...
    DataModel::TestSuite *ts =
            _processControl->loadTestSuiteObject(path.toStdString());
    if (!ts) {
...
void HMITesterControl::action_new_triggered() {
    ...
    if (result) {
        // call the controller method
        bool ok =
                _processControl->newTestSuite(newtsd.getTestsuitePath().toStdString(),
...
void HMITesterControl::action_showTesterOnTop_triggered(bool b) {
    ...
    w->setParent(dynamic_cast<QWidget *>(w->parent()),
...
void HMITesterControl::updateTestSuiteInfo(DataModel::TestSuite *ts) {
    ...
    if (ts != NULL) {
        ...
        if (ts->count() > 0) {
            ...
            for (tcit = ts->testCases().begin(); tcit != ts->testCases().end();
                 tcit++) {
...
void HMITesterControl::setForm_initState() { emit form_initState(); }
...
void HMITesterControl::setForm_stopState() { emit form_stopState(); }
...
void HMITesterControl::setForm_playState() { emit form_playState(); }
...
void HMITesterControl::setForm_recState() { emit form_recState(); }
...
void HMITesterControl::setForm_pausePlayState() { emit form_pausePlayState(); }
...
void HMITesterControl::setForm_pauseRecState() { emit form_pauseRecState(); }
